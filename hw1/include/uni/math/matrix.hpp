...
namespace uni::math
{
  ...
  public:
    ...
    constexpr static matrix<Rows, Cols> zero()
    {
      return matrix<Rows, Cols>{};
    }

    template<std::size_t r = Rows, std::size_t c = Cols,
	     typename = std::enable_if<(c == r
					&& r == Rows
					&& c == Cols)>::type>
  ...
  constexpr matrix<Rows, Cols> operator+(const matrix<Rows, Cols>& lhs, const matrix<Rows, Cols>& rhs)
  {
  ...
  constexpr matrix<Rows, Cols> operator*(const matrix<Rows, K>& lhs, const matrix<K, Cols>& rhs)
  {
    matrix<Rows, Cols> result;
    for (std::size_t row = 0; row < Rows; ++row) {
      for (std::size_t col = 0; col < Cols; ++col) {
  ...
  template<std::size_t Rows, std::size_t Cols>
  constexpr matrix<Rows, Cols> operator*(const typename matrix<Rows, Cols>::value_type number,
  ...
  template<std::size_t Rows, std::size_t Cols>
  std::ostream& operator<<(std::ostream& out, const uni::math::matrix<Rows, Cols>& matrix)
  {
    for (std::size_t i = 0; i < Rows; ++i) {
      for (std::size_t j = 0; j < Cols; ++j) {
	out << matrix.at(i, j) << ' ';
